...
class SQwen2ProcessingInfo(BaseProcessingInfo):
    def get_supported_mm_limits(self) -> Mapping[str, Optional[int]]:
        return {
            "audio": MAX_N_CHUNKS,
        }

class SQwen2Processor:
    def __init__(
        self,
...
class SQwen2DummyInputsBuilder(
        BaseDummyInputsBuilder[SQwen2ProcessingInfo]):
    ...
    def get_dummy_mm_data(
        ...
        num_chunks = mm_counts.get("audio", 0)
        dummy_feature = torch.zeros((CHUNK_SIZE, 3584), dtype=torch.float32) # unique for qwen2.5-7b
        return {
            "audio": [dummy_feature] * num_chunks,
        }


class SQwen2MultiModalProcessor(
        BaseMultiModalProcessor[SQwen2ProcessingInfo]):
    
    def get_hf_processor(self, **kwargs: object) -> SQwen2Processor:
...
@MULTIMODAL_REGISTRY.register_processor(
    SQwen2MultiModalProcessor,
    info=SQwen2ProcessingInfo,
    dummy_inputs=SQwen2DummyInputsBuilder)
class SQwen2ForConditionalGeneration(nn.Module, SupportsMultiModal,
                                         SupportsPP):
